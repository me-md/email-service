...
RSpec.describe type: :mailer do
	...
	let(:mail) { InfoMailer.send_record("eeeee@evette.com", data).deliver_now }

	it 'sends an email' do
    expect { InfoMailer.send_record("eeeee@email.com", data).deliver_now }
      .to change { ActionMailer::Base.deliveries.count }.by(1)
	end
	
	it 'has properties' do
		expect(mail.subject).to eq('Your MeMD Results')
		expect(mail.to).to eq(['eeeee@evette.com'])
	end
end